# KRIVIČNE PRIJAVE – KOMPLETAN PAKET DOKAZA
## Criminal Complaints Package – Complete Evidence

**Pripremljeno:** 21. novembar 2025  
**Status:** Dokumentovano i dostupno  
**Verzija:** 4.0 – finalna, 30 screenshot dokaza  
**Dokumentacija:** 10 datoteka + manifest

---

## SADRŽAJ PAKETA / COMPLETE PACKAGE CONTENTS

### GLAVNE KRIVIČNE PRIJAVE (4)

1. **PRIJAVA_I_KORUPCIJA.md**
   - Tema: sistemska korupcija u zdravstvu i socijalnoj zaštiti
   - Dokazi: 13 smrti u požarima domova bez odobrenja, 60.000 građana bez adekvatne nege
   - Adresat: Viši javni tužilac u Beogradu
   - Dužina: ~45 strana sa dokazima
   - Status: kompletna, 17 screenshot dokaza

2. **PRIJAVA_II_ORGANIZOVANI_KRIMINAL.md**  
   - Tema: organizovana kriminalna grupa i finansijski tokovi  
   - Vrednost: oko 50M+ evra spornih transakcija / privatizacija  
   - Adresat: Tužilaštvo za organizovani kriminal  
   - Dužina: ~35 strana  
   - Status: kompletna, 5 screenshot dokaza

3. **PRIJAVA_III_POLITIČKA_ODGOVORNOST.md**
   - Tema: politička odgovornost i mehanizmi imuniteta
   - Dokazi: 13 smrti u požarima domova bez odobrenja, 60.000 građana isključenih iz sistema
   - Adresat: Republičko javno tužilaštvo + relevantne EU institucije
   - Dužina: ~50 strana
   - Status: kompletna, 22 screenshot dokaza

4. **PRIJAVA_IV_EU_FONDOVI_MEĐUNARODNO.md**  
   - Tema: zloupotreba EU fondova i međunarodna dimenzija  
   - Vrednost: procena preko 450M evra (IPA III fondovi)  
   - Adresat: OLAF, Interpol, Evropska komisija  
   - Dužina: ~40 strana  
   - Status: kompletna, 17 screenshot dokaza

### POMOĆNI DOKUMENTI (6)

5. **PRILOG_LEGAL_APPENDIX_EVIDENCE.md**  
   - Formalni pravni citati i bibliografske reference  
   - 10+ ključnih izvora sa direktnim citatima  
   - Status: kompletan

6. **INDEKS_DOKAZA_KROSREFERENCE_PRIJAVE.md**  
   - Indeks sa 50+ mapiranja dokaza  
   - Krosreference između sve četiri prijave  
   - Status: kompletan

7. **EVIDENCE_COLLECTION_BRNABIĆ_LONČAR_MALI.md**  
   - Analiza u 10 sekcija (384 reda)  
   - Veze između tri zvaničnika kroz 15+ godina  
   - Status: kompletan

8. **INTEGRATION_SUMMARY_BRNABIĆ_LONČAR_MALI.md**  
   - Kako online dokazi jačaju prijave  
   - Pet ključnih povezanosti za brzo razumevanje  
   - Status: kompletan

9. **FINAL_REVIEW_ALL_COMPLAINTS.md**  
   - Provera kvaliteta za sve prijave  
   - Verifikacija izvora, konzistentnost, procena rizika (LOW)  
   - Status: kompletan

10. **FILING_PREPARATION_COMPLETE.md**  
    - Finalna priprema dokumentacije  
    - Checklista i koraci za pakovanje dokaza  
    - Status: kompletan

### SCREENSHOT DOKAZI (30)

- Lokacija: `.playwright-mcp/`  
- Ukupno: 30 PNG fajlova, ~50 MB  
- Dokumentacija: `SCREENSHOT_EVIDENCE_MANIFEST.md`

---

## DOKAZI PO PRIJAVI

| Prijava | Primarni | Podržavajući | Ukupno | Kvalitet |
|---------|----------|--------------|--------|----------|
| **PRIJAVA_I** (Korupcija) | 12 | 5 | **17** | vrlo visok |
| **PRIJAVA_II** (Org. kriminal) | 3 | 2 | **5** | vrlo visok |
| **PRIJAVA_III** (Politička odg.) | 14 | 8 | **22** | veoma visok |
| **PRIJAVA_IV** (EU fondovi) | 10 | 7 | **17** | vrlo visok |
| **UKUPNO** | **39** | **22** | **30 jedinstvenih** | **svi visoki** |

---

## ORGANIZACIJA SCREENSHOT FOLDERA

### Batch 1 (8 screenshot-a) – inicijalne istrage
- KRIK: Mali, 45 bankovnih računa
- BIRN: respiratori, 8.9M
- Danas/DRI: 871 dan čekanja
- Balkan Insight: inspektori požara
- CTIF: Barajevo, smrti u požaru domova bez odobrenja
- EU Progress Report 2024
- DRI: institucionalna baza
- Vojka: suđenje za požar

### Batch 2 (8 screenshot-a) – vlada i institucije
- RTS Politika, početna  
- RTS, zapisnik sednice vlade  
- RTS: Vučić–Mali EXPO  
- A11: Zemun Polje, smeštaj  
- MDRI-S institucionalne studije  
- DRI studije  
- Dečje Srce, model zajedničke nege  
- Balkan Insight „Worst Nightmare“

### Batch 3 (14 screenshot-a) – finalni set (21. novembar 2025)
- Amnesty „Trapped by Automation“ (2)  
- OCCRP: Lončar – veze sa organizovanim kriminalom  
- Balkan Insight: deinstitucionalizacija  
- A11: Zemun Polje  
- MDRI-S / DRI partnerstvo  
- Dečje Srce  
- Balkan Insight „Worst Nightmare“  
- HRW COVID – institucije  
- Amnesty izveštaj (celovit)  
- Statistički zavod – invaliditet  
- MINRZS – socijalna pomoć  
- EC Serbia Report 2024  
- GRECO – procena korupcije

---

## STATUS CHECKLIST

### Dokumenti – 100% kompletni
- PRIJAVA_I_KORUPCIJA.md  
- PRIJAVA_II_ORGANIZOVANI_KRIMINAL.md  
- PRIJAVA_III_POLITIČKA_ODGOVORNOST.md  
- PRIJAVA_IV_EU_FONDOVI_MEĐUNARODNO.md

### Pomoćni dokumenti – 100% kompletni
- PRILOG_LEGAL_APPENDIX_EVIDENCE.md  
- INDEKS_DOKAZA_KROSREFERENCE_PRIJAVE.md  
- EVIDENCE_COLLECTION_BRNABIĆ_LONČAR_MALI.md  
- INTEGRATION_SUMMARY_BRNABIĆ_LONČAR_MALI.md  
- FINAL_REVIEW_ALL_COMPLAINTS.md  
- FILING_PREPARATION_COMPLETE.md  
- SCREENSHOT_EVIDENCE_MANIFEST.md

### Screenshot dokazi – 100% kompletni
- 30 online captura  
- Organizovane i katalogizovane  
- Mapirane na konkretne prijave  
- Procena pravne branljivosti: visoka

### Pravna priprema – 100% kompletna
- Sve tvrdnje potkrepljene izvorima  
- Izvori istraženi i verifikovani  
- Bez kontradikcija između dokumenata  
- Jezik prilagođen pravnom postupku  
- Bez spekulacija ili pretpostavki  
- Procena rizika: niska

---

## PREPORUČENI REDOSLED ČITANJA

### Faza 1 – domaća osnova
1. **PRIJAVA_III_POLITIČKA_ODGOVORNOST** – imunitet i okvir odgovornosti (22 screenshot-a)

### Faza 2 – kriminalna dimenzija
2. **PRIJAVA_I_KORUPCIJA** – finansijska koordinacija i zloupotrebe (17 screenshot-a)  
3. **PRIJAVA_II_ORGANIZOVANI_KRIMINAL** – struktura grupe i tokovi novca (5 screenshot-a)

### Faza 3 – međunarodni pritisak
4. **PRIJAVA_IV_EU_FONDOVI_MEĐUNARODNO** – EU fondovi, OLAF i EU Komisija (17 screenshot-a)

---

## CC PRIMAOCE (SVE PRIJAVE)

- OLAF (EU Anti-Fraud Office)  
- EUROPOL (Organizovani kriminal)  
- EU Delegation Serbia  
- GRECO (Council of Europe)  
- Amnesty International  
- OCCRP

---

## STRUKTURA FAJLOVA

```
prijave/
├── README.md
├── PRIJAVA_I_KORUPCIJA.md
├── PRIJAVA_II_ORGANIZOVANI_KRIMINAL.md
├── PRIJAVA_III_POLITIČKA_ODGOVORNOST.md
├── PRIJAVA_IV_EU_FONDOVI_MEĐUNARODNO.md
├── PRILOG_LEGAL_APPENDIX_EVIDENCE.md
├── INDEKS_DOKAZA_KROSREFERENCE_PRIJAVE.md
├── EVIDENCE_COLLECTION_BRNABIĆ_LONČAR_MALI.md
├── INTEGRATION_SUMMARY_BRNABIĆ_LONČAR_MALI.md
├── FINAL_REVIEW_ALL_COMPLAINTS.md
├── FILING_PREPARATION_COMPLETE.md
└── SCREENSHOT_EVIDENCE_MANIFEST.md

.playwright-mcp/ (30 PNG fajlova)
└── 01-30 (po batch kolekcijama, dokumentovano u manifestu)
```

---

## REFERENTNI LINKOVI

- [Viši javni tužilac](https://www.tužilac.rs)  
- [OLAF](https://anti-fraud.ec.europa.eu)  
- [EUROPOL](https://www.europol.europa.eu)  
- [GRECO](https://www.coe.int/greco)  
- [EU Delegation Serbia](https://ec.europa.eu/delegations/serbia)

---

## KAKO KORISTITI OVAJ PAKET

### Za tužioce:
1. Počnite sa `FILING_PREPARATION_COMPLETE.md`  
2. Čitajte `PRIJAVA_III` za okvir imuniteta  
3. Koristite `PRILOG_LEGAL_APPENDIX_EVIDENCE.md` za citate  
4. Priložite svih 30 screenshot-ova

### Za međunarodne institucije:
1. `FINAL_REVIEW_ALL_COMPLAINTS.md` (QA provera)  
2. `SCREENSHOT_EVIDENCE_MANIFEST.md` (pregled slika)  
3. `INDEKS_DOKAZA_KROSREFERENCE_PRIJAVE.md` (brzo pretraživanje)

### Za novinare:
1. `EVIDENCE_COLLECTION_BRNABIĆ_LONČAR_MALI.md` (analiza)  
2. Screenshot-ovi iz `.playwright-mcp/`  
3. COVID-19 koordinacija (Sekcija 2)  
4. Lončar–Zemun veze (Sekcija 3)

---

**Status:** dokumentovano  
**Verzija:** 4.0 – finalna  
**Datum:** 21. novembar 2025
